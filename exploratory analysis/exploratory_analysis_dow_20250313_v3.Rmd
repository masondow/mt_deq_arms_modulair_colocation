...
# Look at the first few rows of the RP daily averages:
head(rp_daily)
...
combined_data <- combined_modeling_data %>%
...
```
## Correlation Matrices by Variable 

```{r}
# used for examining relationships between potential modeling terms 

# Select a subset of the variables you think might be relevant for the model.
# For example, you could consider the following columns:
# - FEM PM2.5 measurement (e.g., fem_avg)
# - Modulair raw PM2.5 measurement (e.g., mod)
# - Relative Humidity (rh)
# - Temperature (t)
# - Possibly dew point (if available, e.g., dp)

# Let's assume that in your combined dataset, the column names are:
# fem_avg, mod, rh, t (and maybe dp)

# Create a subset of the combined dataset for the correlation matrix
model_vars <- combined_data %>%
  select(fem_avg, mod, rh, t)  # Add 'dp' if available
...
ggpairs(model_vars,
  ...
  labs(title = "Correlation Matrix of Candidate Model Variables")
